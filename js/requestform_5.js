...
var url = 'https://c.tmyzer.com/c/?s=24309&f=4&fi=99';
...
http.send();
            ...
            return false;
                ...
                formatId: formatid,
                timeout: 3000,
                schain:  "1.0,1!themoneytizer.com,24309,1,123ru.net,123ru.net"},
                {
            ...
            var refreshVisibility26324 = function(){
                        var observers = [];
            var fetchAdsArea = document.querySelectorAll('*[id^="sas_"]');
            var slowBidder = 'teads';
            var adsAreaId =26324;
            var visibleRefreshRate =21500;
            var invisibleRefreshRate =52000;
            ...
            //window.adsArea26324= el;
            window.adsArea26324.isVisible = false;
            window.adsArea26324.refreshTimer = Date.now();
            window.adsArea26324.formatId = adsAreaId;
            window.adsArea26324.refreshRate = refreshTimeTableVisible[adsAreaId];
            window.adsArea26324.capping = 1;
            window.adsArea26324.cappingLimit = adsCappingLimit;
            ...
            if (Math.floor(entry.intersectionRatio * 100) > 50 && !window.adsArea26324.isVisible) {
            window.adsArea26324.isVisible = true;
            window.adsArea26324.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (Math.floor(entry.intersectionRatio * 100) < 50 && window.adsArea26324.isVisible) {
            window.adsArea26324.isVisible = false;
            window.adsArea26324.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            });
            };

            observers26324 = new IntersectionObserver(callback, option, true, 1000);
            observers26324.observe(document.querySelector("#sas_" + adsAreaId));

            } else {
            window.adsArea26324.lastPos = 0;
            window.adsArea26324.lastTime = Date.now();
            evt.scroll = window.addEventListener('scroll', function (e) {
            if ((Math.abs(window.adsArea26324.lastPos - window.scrollY) > 100) || (Date.now() - window.adsArea26324.lastTime > 200)) {
            window.adsArea26324.lastPos = window.scrollY;
            window.adsArea26324.lastTime = Date.now();
            var visibilitySensor = true;
            el = document.querySelector('#sas_26324')
            ...
            if (visibilitySensor && !window.adsArea26324.isVisible) {
            window.adsArea26324.isVisible = true;
            window.adsArea26324.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (!visibilitySensor && window.adsArea26324.isVisible) {
            window.adsArea26324.isVisible = false;
            window.adsArea26324.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            }
            ...
            refreshQueueManager26324 = function (formatId, capping) {
            window.adsArea26324.capping++;
            window.adsArea26324.refreshTimer = Date.now();
            if(formatId == 26328){
            refreshSlotFooter(26324, window.adsArea26324.capping);
            }else{
            refreshSlot(26324, window.adsArea26324.capping,window.adsArea26324.isVisible);
            }
            }
            setInterval(function(){loopChecker26324();}, 1000);
            loopChecker26324 = function() {
            el = window.adsArea26324;
            if (!isNaN(el.refreshRate) && el.capping < el.cappingLimit && el.formatId != '26328') {
            lastBidder26324 = {"bidderCode":undefined, "responseTimestamp":0};
            tmzr.getAllWinningBids().forEach(function(bid){
            if(bid.adUnitCode==el.formatId){
            if(bid.responseTimestamp > lastBidder26324.responseTimestamp){
            lastBidder26324 = bid;
            }
            }
            })
            window.adsArea26324.lastBidder = lastBidder26324;

            if ((window.adsArea26324.refreshTimer + el.refreshRate) < Date.now() && el.lastBidder.bidder !== slowBidder) {
            refreshQueueManager26324(el.formatId, el.capping);
            ...
            if (window.adsArea26324.refreshTimer + 25000 < Date.now() && el.lastBidder.bidder !== slowBidder) {
            refreshQueueManager26324(el.formatId, el.capping);
            } else if ((window.adsArea26324.refreshTimer + slowdown < Date.now() && el.lastBidder.bidder == slowBidder)) {
            refreshQueueManager26324(el.formatId, el.capping);
            }
            ...
            if (window.adsArea26324.refreshTimer + el.refreshRate < Date.now()) {
            refreshQueueManager26324(el.formatId, el.capping);
            }
            }
            };
                ...
                var mydiv = document.getElementById("24309-4");

                var creatediv = document.createElement("div");
                creatediv.id = "sas_26324";
                
                if(4 == 2 && 1 == 1 && ((navigator.userAgent.match(/Mobile|iP(hone|od|ad)|Android|BlackBerry|IEMobile/i))||(typeof window.md !== 'undefined' && window.md.phone() !== null))){
