...
var url = 'https://c.tmyzer.com/c/?s=24309&f=5&fi=99';
...
http.send();
            ...
            if ((window.adsArea26325.refreshTimer + el.refreshRate) < Date.now() && el.lastBidder.bidder !== slowBidder) {
            ...
            if (window.adsArea26325.refreshTimer + 25000 < Date.now() && el.lastBidder.bidder !== slowBidder) {
            refreshQueueManager26325(el.formatId, el.capping);
            } else if ((window.adsArea26325.refreshTimer + slowdown < Date.now() && el.lastBidder.bidder == slowBidder)) {
            refreshQueueManager26325(el.formatId, el.capping);
            }
            ...
            refreshQueueManager26325(el.formatId, el.capping);
                    ...
                    var target = document.getElementsByTagName('head')[0];
                    target.appendChild(skinjs);
                                if(typeof crtg_content === 'undefined'){var crtg_content = "";}
                var mydiv = document.getElementById("24309-5");
                ...
                sc.text = sas.cmd.push(function(){sas.call('std',{siteId: 279931, pageId: 1035006, formatId: 26325, target: crtg_content+moneybid});});
                creatediv.appendChild(sc);
                var http = new XMLHttpRequest();
                var url = 'https://c.tmyzer.com/c/?s=24309&f=5&fi=0';
                http.open("GET", url, true);
                http.send();
                }
                </l;i++)>